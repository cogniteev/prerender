...
var USER_AGENT = process.env.USER_AGENT || null;

var MONITOR_RESOURCES = process.env.MONITOR_RESOURCES == "true" || false;
...
server.onPhantomPageCreate = function(req, res) {
    var _this = this;
    var args = {
        blockedResources: _this.options.blockedResources || blockedResources,
        logRequests: !!_this.options.logRequests,
        userAgent: USER_AGENT,
        monitorResources: MONITOR_RESOURCES
    };
    ...
    req.prerender.page.run(args, function(options, resolve, reject) {
        var _this = this;
        this.prerender = {
            resourcesRequested: [],
            resourcesReceived: [],
        ...
        this.onResourceRequested = function(requestData, request) {
            if (options.monitorResources) {
                _this.prerender.resourcesData[requestData.url] = {
                    start: null,
                    ttfb: null,
                    ttlb: null,
                    status: null,
            ...
            for(var i = 0,l = options.blockedResources.length; i < l; i++) {
                var regex = new RegExp(options.blockedResources[i], 'gi');
                if(regex.test(requestData.url)) {
                    request.abort();
                    requestData.aborted = true;
                    if (options.monitorResources) {
                        _this.prerender.resourcesData[requestData.url].status = "blocked";
                    }
                    break;
                }
            }
            ...
            if(!requestData.aborted) {
                ...
                if (options.monitorResources) {
                    _this.prerender.resourcesData[requestData.url].start = new Date().getTime();
                }
        ...
        this.onResourceReceived = function(response) {
            ...
            if (options.monitorResources) {
                var monitoredResource = _this.prerender.resourcesData[response.url];
                if (response.stage === 'start') {
                    monitoredResource.ttfb = response.time.getTime() - monitoredResource.start;
                }
                if (response.stage === 'end') {
                    monitoredResource.ttlb = response.time.getTime() - monitoredResource.start - monitoredResource.ttfb;
                    monitoredResource.status = "success";
                    monitoredResource.responseHeaders = response.headers;
                    monitoredResource.statusCode = response.status;
                    monitoredResource.contentType = response.contentType;
                } else {
                    // bodySize is transferred size
                    // if server does chunk encoding, we need to sum each chunk
                    monitoredResource.size = (monitoredResource.size||0) + response.bodySize;
                }
            }
        ...
        this.onResourceTimeout = function(request) {
            if(request.url) {
                ...
                if (options.monitorResources) {
                    _this.prerender.resourcesData[request.url].status = "timeout";
                }
            }
        }
        ...
        this.onResourceError = function(resourceError) {
            if(resourceError.url) {
                if (options.monitorResources) {
                    _this.prerender.resourcesData[resourceError.url].status = "error";
                }
                if (options.logRequests) {
        ...
        if (options.userAgent) {
            this.settings.userAgent = options.userAgent;
        } else {
...
server.checkIfPageIsDoneLoading = function(req, res) {
    ...
    req.prerender.page.run(function(resolve) {
        ...
        req.prerender.resourcesData = response.resourcesData;
...
server._sendResponse = function(req, res, options) {
    ...
    if (MONITOR_RESOURCES && req.prerender.resourcesData) {
        var renderedPage = req.prerender.resourcesData[req.prerender.url];
        renderedPage.onLoad = req.prerender.lastJavascriptExecution.getTime();
        renderedPage.rendering = renderedPage.onLoad - renderedPage.start;
        res.setHeader('X-Resources-Timeline', JSON.stringify(req.prerender.resourcesData));
    }
