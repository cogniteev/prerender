    ...
    req.prerender.page.run(args, function(options, resolve, reject) {
        ...
        this.onResourceRequested = function(requestData, request) {
            ...
            if(!requestData.aborted) {
                ...
                if (options.monitorResources) {
                    _this.prerender.resourcesData[requestData.url].start = new Date();
                }
        ...
        this.onResourceReceived = function(response) {
            ...
            if (options.monitorResources) {
                if (response.stage === 'start') {
                    _this.prerender.resourcesData[response.url].startReply = response.time;
                }
                if (response.stage === 'end') {
                    _this.prerender.resourcesData[response.url].end = response.time;
                    _this.prerender.resourcesData[response.url].status = "success";
                    _this.prerender.resourcesData[response.url].responseHeaders = response.headers;
                    _this.prerender.resourcesData[response.url].statusCode = response.status;
                    _this.prerender.resourcesData[response.url].contentType = response.contentType;
                } else {
                    // bodySize is transferred size
                    // if server does chunk encoding, we need to sum each chunk
                    _this.prerender.resourcesData[response.url].size =
                    (_this.prerender.resourcesData[response.url].size||0) + response.bodySize;
                }
            }
...
server._sendResponse = function(req, res, options) {
    ...
    if (MONITOR_RESOURCES && req.prerender.resourcesData) {
        req.prerender.resourcesData[req.prerender.url].onLoad = req.prerender.lastJavascriptExecution;
